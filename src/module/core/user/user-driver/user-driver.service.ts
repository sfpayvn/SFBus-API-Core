...
import { UpdatePasswordUserDto, UpdateUserProfileDto } from '../user/dto/update-user.dto';
...
export class UserDriverService {
  ...
  async create(createUserDto: CreateUserDto, tenantId?: Types.ObjectId): Promise<UserDto> {
    createUserDto.roles = [ROLE_CONSTANTS.DRIVER];
  ...
  async search(
    ...
    if (matchStageIndex !== -1) {
      ...
      pipeline[matchStageIndex].$match = {
        ...pipeline[matchStageIndex].$match,
        roles: { $in: [ROLE_CONSTANTS.DRIVER] },
      };
    } else {
      // If no $match stage, add one at the beginning
      pipeline.unshift({
        $match: { roles: { $in: [ROLE_CONSTANTS.DRIVER] } },
      });
    }
    ...
    // Đếm tổng số mục
    const totalItem = await this.userModel.countDocuments({ tenantId, roles: { $in: [ROLE_CONSTANTS.DRIVER] } });
