 ...
 */
@Injectable()
export class UserTenantOperatorService {
  constructor(
    @InjectModel(UserDocument.name) private userModel: Model<UserDocument>,
    private readonly userService: UserService,
  ) {}

  async create(adminCreateUserDto: CreateUserDto, tenantId?: Types.ObjectId): Promise<UserDto> {
    adminCreateUserDto.roles = [ROLE_CONSTANTS.TENANT_OPERATOR];
    return this.userService.create(adminCreateUserDto, tenantId);
  }

  // Cập nhật thông tin người dùng
  async update(adminUpdateUserProfileDto: UpdateUserProfileDto, tenantId: Types.ObjectId): Promise<UserDto> {
  ...
  async search(
    ...
    if (matchStageIndex !== -1) {
      ...
      pipeline[matchStageIndex].$match = {
        ...pipeline[matchStageIndex].$match,
        roles: { $in: [ROLE_CONSTANTS.TENANT_OPERATOR] },
      };
    } else {
      // If no $match stage, add one at the beginning
      pipeline.unshift({
        $match: { roles: { $in: [ROLE_CONSTANTS.TENANT_OPERATOR] } },
      });
    }
