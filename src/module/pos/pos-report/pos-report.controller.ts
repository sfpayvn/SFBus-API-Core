...
export class PosReportController {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(ROLE_CONSTANTS.POS, ROLE_CONSTANTS.ADMIN)
  @Post('schedule/:busScheduleId')
  async getScheduleReportDetail(
    @Param('busScheduleId', ParseObjectIdPipe) busScheduleId: Types.ObjectId,
    @Query(ParseObjectIdPipe) query: ScheduleReportQueryDto,
    @CurrentUser() user: any,
  ) {
    return this.reportService.getScheduleReportDetail(busScheduleId, new Types.ObjectId(user.tenantId), query);
  }
}
