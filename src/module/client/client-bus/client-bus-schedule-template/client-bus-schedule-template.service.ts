...
import {
  ClientBusScheduleTemplateDto,
  ClientSearchBusScheduleTemplateQuery,
  ClientSearchBusScheduleTemplateQuerySortFilter,
} from './dto/client-bus-schedule-template.dto';

@Injectable()
export class ClientBusScheduleTemplateService {
  ...
  async findAll(tenantIds: Types.ObjectId[]): Promise<ClientBusScheduleTemplateDto[]> {
    return this.busScheduleTemplateService.findAll(tenantIds);
  }

  async findOne(id: Types.ObjectId, tenantIds: Types.ObjectId[]): Promise<ClientBusScheduleTemplateDto> {
