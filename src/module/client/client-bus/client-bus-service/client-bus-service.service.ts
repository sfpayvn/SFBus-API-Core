...
import {
  ClientBusServiceDto,
  ClientSearchBusServicesQuerySortFilter,
  ClientSearchBusServicesRes,
} from './dto/client-bus-service.dto';

@Injectable()
export class ClientBusServiceService {
  ...
  async findAll(tenantIds: Types.ObjectId[]): Promise<ClientBusServiceDto[]> {
    return this.busServiceService.findAll(tenantIds);
  }

  async findOne(id: Types.ObjectId, tenantIds: Types.ObjectId[]): Promise<ClientBusServiceDto> {
  ...
  ): Promise<ClientSearchBusServicesRes> {
