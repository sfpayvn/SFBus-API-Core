"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AdminSeatTypeService = void 0;
const seat_type_schema_1 = require("../../../core/seat/seat-type/schema/seat-type.schema");
const seat_type_service_1 = require("../../../core/seat/seat-type/seat-type.service");
const common_1 = require("@nestjs/common");
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
let AdminSeatTypeService = class AdminSeatTypeService {
    constructor(seatTypeModel, seatTypeService) {
        this.seatTypeModel = seatTypeModel;
        this.seatTypeService = seatTypeService;
        this.ROOT_TENANT_ID = process.env.ROOT_TENANT_ID?.trim() || '';
    }
    async create(adminCreateSeatTypeDto, tenantId) {
        return this.seatTypeService.create(adminCreateSeatTypeDto, tenantId);
    }
    async update(adminUpdateSeatTypeDto, tenantId) {
        return this.seatTypeService.update(adminUpdateSeatTypeDto, tenantId);
    }
    async delete(id, tenantId) {
        return this.seatTypeService.delete(id, tenantId);
    }
    async findAll(tenantIds) {
        return this.seatTypeService.findAll(tenantIds);
    }
    async findOne(id, tenantIds) {
        return this.seatTypeService.findOne(id, tenantIds);
    }
    async search(pageIdx, pageSize, keyword, sortBy, filters, tenantIds) {
        return this.seatTypeService.search(pageIdx, pageSize, keyword, sortBy, filters, tenantIds);
    }
};
exports.AdminSeatTypeService = AdminSeatTypeService;
exports.AdminSeatTypeService = AdminSeatTypeService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(seat_type_schema_1.SeatTypeDocument.name)),
    __param(1, (0, common_1.Inject)((0, common_1.forwardRef)(() => seat_type_service_1.SeatTypeService))),
    __metadata("design:paramtypes", [mongoose_2.Model,
        seat_type_service_1.SeatTypeService])
], AdminSeatTypeService);
//# sourceMappingURL=admin-seat-type.service.js.map