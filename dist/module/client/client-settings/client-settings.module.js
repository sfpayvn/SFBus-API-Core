"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ClientSettingsModule = void 0;
const common_1 = require("@nestjs/common");
const client_settings_controller_1 = require("./client-settings.controller");
const settings_module_1 = require("../../core/settings/settings.module");
const client_tenant_module_1 = require("../client-tenant/client-tenant.module");
const tenant_module_1 = require("../../core/tenant/tenant.module");
const app_version_service_1 = require("./services/app-version.service");
let ClientSettingsModule = class ClientSettingsModule {
};
exports.ClientSettingsModule = ClientSettingsModule;
exports.ClientSettingsModule = ClientSettingsModule = __decorate([
    (0, common_1.Module)({
        imports: [settings_module_1.SettingsModule, (0, common_1.forwardRef)(() => client_tenant_module_1.ClientTenantModule), (0, common_1.forwardRef)(() => tenant_module_1.TenantModule)],
        controllers: [client_settings_controller_1.ClientSettingsController],
        providers: [app_version_service_1.ClientAppVersionService],
        exports: [app_version_service_1.ClientAppVersionService],
    })
], ClientSettingsModule);
//# sourceMappingURL=client-settings.module.js.map