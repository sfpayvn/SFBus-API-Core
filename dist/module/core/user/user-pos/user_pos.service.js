...
let UserPosService = class UserPosService {
    constructor(userModel, userService) {
        this.userModel = userModel;
        this.userService = userService;
    }
    async create(createUserDto, tenantId) {
        createUserDto.roles = [roles_constants_1.ROLE_CONSTANTS.POS];
    ...
    async search(pageIdx, pageSize, keyword, sortBy, filters, tenantId) {
        ...
        if (matchStageIndex !== -1) {
            pipeline[matchStageIndex].$match = {
                ...pipeline[matchStageIndex].$match,
                roles: { $in: [roles_constants_1.ROLE_CONSTANTS.POS] },
            };
        }
        else {
            pipeline.unshift({
                $match: { roles: { $in: [roles_constants_1.ROLE_CONSTANTS.POS] } },
            });
        }
        const usersModel = await this.userModel.aggregate(pipeline).exec();
        const totalItem = await this.userModel.countDocuments({ tenantId, roles: { $in: [roles_constants_1.ROLE_CONSTANTS.POS] } });
...
exports.UserPosService = UserPosService;
exports.UserPosService = UserPosService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, mongoose_1.InjectModel)(user_schema_1.UserDocument.name)),
    __metadata("design:paramtypes", [mongoose_2.Model,
        user_service_1.UserService])
], UserPosService);
//# sourceMappingURL=user_pos.service.js.map