...
let UserClientService = class UserClientService {
    ...
    async create(createUserDto, tenantId) {
        createUserDto.roles = [roles_constants_1.ROLE_CONSTANTS.CLIENT];
    ...
    async search(pageIdx, pageSize, keyword, sortBy, filters, tenantId) {
        ...
        if (matchStageIndex !== -1) {
            pipeline[matchStageIndex].$match = {
                ...pipeline[matchStageIndex].$match,
                roles: { $in: [roles_constants_1.ROLE_CONSTANTS.CLIENT] },
            };
        }
        else {
            pipeline.unshift({
                $match: { roles: { $in: [roles_constants_1.ROLE_CONSTANTS.CLIENT] } },
            });
        }
        const usersModel = await this.userModel.aggregate(pipeline).exec();
        const totalItem = await this.userModel.countDocuments({ tenantId, roles: { $in: [roles_constants_1.ROLE_CONSTANTS.CLIENT] } });
...
//# sourceMappingURL=user_client.service.js.map